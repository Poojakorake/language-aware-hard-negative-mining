...
%-------------------------------------------------------------------------
\begin{abstract}
Hard negative mining is central to learning discriminative vision-language embeddings.
Existing approaches such as LLaVE compute hardness solely from embedding-space similarity,
which can conflate visually distinct but semantically related negatives.
We propose a simple yet effective extension that augments the hardness score with a \emph{linguistic similarity} term derived from a frozen Sentence-BERT model.
The combined score $H_{i,j} = \alpha \cdot \text{sim}_\text{embed}(i,j) + \beta \cdot \text{sim}_\text{ling}(i,j)$ re-weights the contrastive loss denominator to penalise pairs that are both embedding-close \emph{and} textually similar, preventing the model from treating semantically equivalent captions as hard negatives.
We evaluate on the Flickr30K 1K gallery using Recall@$K$ and Median Rank across five ($\alpha$, $\beta$) configurations.
We identify an optimal balance at $\alpha{=}5$, $\beta{=}5$, which yields consistent gains over the embedding-only baseline, improving Recall@10 from 25.16\% to 30.64\% and reducing the Median Rank from 47 to 33.
\end{abstract}
...
We presented a language-aware hard negative mining strategy that augments embedding-space hardness with a frozen Sentence-BERT linguistic similarity signal.
The combined hardness score prevents semantically equivalent captions from being treated as hard negatives.
Our best-performing model ($\alpha{=}5$, $\beta{=}5$) achieved a 21.8\% relative improvement in Recall@10 (25.16\%$\to$30.64\%) and reduced the Median Rank from 47 to 33 on the Flickr30K 1K gallery, using no additional learnable parameters.
An ablation across five ($\alpha$, $\beta$) configurations reveals that equal weighting provides the most robust retrieval signal and that excessively high combined weights lead to gradient conflict.
The method is simple to integrate into any InfoNCE-based vision-language training pipeline and adds negligible overhead beyond a one-time pre-computation step.
