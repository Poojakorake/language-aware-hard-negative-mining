...
In practice, not all negatives are equally useful---if the model can already tell two items apart easily, that pair teaches it nothing.
\emph{Hard} negatives, ones that look similar but are not actually matched, are what push the model to learn the subtle differences that matter.
...
The problem is that using only the model's own embeddings to measure hardness can backfire.
...
We froze Sentence-BERT rather than fine-tuning it because we needed a stable semantic signal---if the linguistic encoder updated during training, the hardness scores would drift and become a moving target.
Keeping it frozen also means the embeddings are computed only once before training and looked up in $O(1)$ per batch, adding no learnable parameters or additional backward passes.
